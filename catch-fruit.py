...
class ScoreBoard():
    ...
    def gameOver(self):
        self.saveScore()
        tkinter.messagebox.showinfo("G0t R3kT Mate ?", "You Just G0t R3kT Mate !")        
        if tkinter.messagebox.askyesno("G0t R3kT Mate ?", "Play Again ?"):
            self.reset()
        else:
            exit()
...
class TheGame(ItemsFallingFromSky,ScoreBoard):
    ...
    def __init__(self,parent):
        ...
        self.parent.title("G0t R3kT Mate ?")
        ...
        # player character
        self.playerPhoto = tkinter.PhotoImage(file = "images/{}" .format( "bunny.gif" ) )
